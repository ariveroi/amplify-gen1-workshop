...
export default function Category() {
  ...
  useEffect(() => {
    ...
    async function queryCategory() {
      try {
        const { data: categories, errors } = await client.graphql({
        ...
        if (categories) {
          // setAllStyles(Object.values(categories.listCategories.items[0].style));
          setCategoryId(categories.listCategories.items[0].id);
          return categories.listCategories.items[0].id;
        }
      } catch (error) {
    ...
    async function queryProduct() {
      try {
        ...
        if (categories.listCategories.items) {
          const { data: products, errors } = await client.graphql({
            query: listProducts,
            variables: {
              filter: {
                categoryID: { eq: categories.listCategories.items[0].id },
              },
            },
          });
          ...
          setProducts(products.listProducts.items);
        }
      } catch (error) {
